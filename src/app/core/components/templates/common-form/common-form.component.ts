...
export class CommonFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.data.data) {
      this.formControl.patchValue(this.data.data as any);
    }
  }
  ...
  onSubmit() {
    ...
    if (this.isAddStock) {
      const stockInfo: any = this.formControl.value;
      stockInfo.code = this.formControl.value.code.toUpperCase();
      stockInfo.startDate = new Date();
      stockInfo.isHoding = true;
      stockInfo.value =
        (this.formControl.value.startPrice as any) *
        (this.formControl.value.volume as any);
